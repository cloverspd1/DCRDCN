namespace BEL.DCRDCNWorkflow.Models.DCR
{
    ...
    public class KAPMarketingInchargeSection : ISection
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="MarketingInchargeSection"/> class.
        /// </summary>
        public KAPMarketingInchargeSection()
        {
        }
        ...
        public KAPMarketingInchargeSection(bool isSet)
        {
            if (isSet)
            {
                this.ListDetails = new List<ListItemDetail>() { new ListItemDetail(DCRDCNListNames.DCRLIST, true) };
                this.SectionName = DCRSectionName.KAPMARKETINGINCHARGESECTION;
        ...
        /// The Marketing User.
        /// </value>
        [DataMember, IsPerson(true, true, true), FieldColumnName("KAPMarketingUser"),  IsViewer]
        ...
        public bool IsSubmitted
        {
            get
            {
                if (this.ApproversList.Any(p => p.Role == DCRRoles.KAPMARKETING && string.IsNullOrEmpty(p.Approver)))
                {
                    return true;
                }
                return false;
