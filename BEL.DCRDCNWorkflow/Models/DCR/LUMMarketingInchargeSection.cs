namespace BEL.DCRDCNWorkflow.Models.DCR
{
    ...
    public class LUMMarketingInchargeSection : ISection
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="MarketingInchargeSection"/> class.
        /// </summary>
        public LUMMarketingInchargeSection()
        {
        }
        ...
        public LUMMarketingInchargeSection(bool isSet)
        {
            if (isSet)
            {
                this.ListDetails = new List<ListItemDetail>() { new ListItemDetail(DCRDCNListNames.DCRLIST, true) };
                this.SectionName = DCRSectionName.LUMMARKETINGINCHARGESECTION;
        ...
        /// The Marketing User.
        /// </value>
        [DataMember, IsPerson(true, true, true), FieldColumnName("LUMMarketingUser"), IsViewer]
        ...
        public bool IsSubmitted
        {
            get
            {
                if (this.ApproversList.Any(p => p.Role == DCRRoles.LUMMARKETING && string.IsNullOrEmpty(p.Approver)))
                {
                    return true;
                }
                return false;
