namespace BEL.DCRDCNWorkflow.Models.DCR
{
    ...
    public class DAPMarketingInchargeSection : ISection
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="MarketingInchargeSection"/> class.
        /// </summary>
        public DAPMarketingInchargeSection()
        {
        }
        ...
        public DAPMarketingInchargeSection(bool isSet)
        {
            if (isSet)
            {
                this.ListDetails = new List<ListItemDetail>() { new ListItemDetail(DCRDCNListNames.DCRLIST, true) };
                this.SectionName = DCRSectionName.DAPMARKETINGINCHARGESECTION;
        ...
        /// The Marketing User.
        /// </value>
        [DataMember, IsPerson(true, true, true), FieldColumnName("DAPMarketingUser"), IsViewer]
        ...
        public bool IsSubmitted
        {
            get
            {
                if (this.ApproversList.Any(p => p.Role == DCRRoles.DAPMARKETING && string.IsNullOrEmpty(p.Approver)))
                {
                    return true;
                }
                return false;
