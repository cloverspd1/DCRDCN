namespace BEL.DCRDCNWorkflow.Models.DCR
{
    ...
    public class LightingMarketingInchargeSection : ISection
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="MarketingInchargeSection"/> class.
        /// </summary>
        public LightingMarketingInchargeSection()
        {
        }
        ...
        public LightingMarketingInchargeSection(bool isSet)
        {
            if (isSet)
            {
                this.ListDetails = new List<ListItemDetail>() { new ListItemDetail(DCRDCNListNames.DCRLIST, true) };
                this.SectionName = DCRSectionName.LIGHTINGMARKETINGINCHARGESECTION;
        ...
        /// The Marketing User.
        /// </value>
        [DataMember, IsPerson(true, true, true), FieldColumnName("LightingMarketingUser"), IsViewer]
        ...
        public bool IsSubmitted
        {
            get
            {
                if (this.ApproversList.Any(p => p.Role == DCRRoles.LIGHTINGMARKETING && string.IsNullOrEmpty(p.Approver)))
                {
                    return true;
                }
                return false;
