namespace BEL.DCRDCNWorkflow.Models.DCR
{
    ...
    public class MorphyRechardMarketingInchargeSection : ISection
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="MarketingInchargeSection"/> class.
        /// </summary>
        public MorphyRechardMarketingInchargeSection()
        {
        }
        ...
        public MorphyRechardMarketingInchargeSection(bool isSet)
        {
            if (isSet)
            {
                this.ListDetails = new List<ListItemDetail>() { new ListItemDetail(DCRDCNListNames.DCRLIST, true) };
                this.SectionName = DCRSectionName.MORPHYRECHARDMARKETINGINCHARGESECTION;
        ...
        /// The Marketing User.
        /// </value>
        [DataMember, IsPerson(true, true, true), FieldColumnName("MRMarketingUser"),  IsViewer]
        ...
        public bool IsSubmitted
        {
            get
            {
                if (this.ApproversList.Any(p => p.Role == DCRRoles.MORPHYRICHARDSMARKETING && string.IsNullOrEmpty(p.Approver)))
                {
                    return true;
                }
                return false;
